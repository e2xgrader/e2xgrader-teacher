...
export default function CellTypeSelector({
  ...
  const options: [string, string][] = [
    ...Object.entries(NbgraderCellTypes.cellTypeLabels),
    ...(cellRegistry
      ?.getPlugins()
      .map(
        (
          plugin: E2xGraderCellRegistry.IE2xGraderCellPlugin
        ): [string, string] => [plugin.cellType, plugin.label]
      ) ?? [])
  ];
  ...
  return (
    <select value={currentType} onChange={e => updateCellType(e.target.value)}>
      <option value="">-</option>
