...
export class DeleteCellCommand implements CommandRegistry.ICommandOptions {
    ...
    execute = async (args: any): Promise<any> => {
        ...
        const selectedCells: GradingCellModel[]|undefined = this._tracker.currentWidget?.content.selectedCells.map(cell => new GradingCellModel(cell.model.sharedModel));
        if(!selectedCells) throw new Error('Unable to delete cells! No cells selected.');

        selectedCells.forEach(selectedCell => findLinkedCells(this._tracker.currentWidget?.content, selectedCell.id).forEach(linkedCell => linkedCells.add(linkedCell)));
